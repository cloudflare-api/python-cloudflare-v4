...
def main():
	...
	for invite in invites:
		...
		print '\t', organization_id, status, invited_member_id, invited_member_email, organization_name, invited_by, invited_on, expires_on
	print ''
	...
	if payment_email is not None:
		print '\t', profile_id, profile_first, profile_last, profile_company, 'PayPal:', payment_email
	else:
		...
		if card_expiry_year is not None and card_expiry_month is not None:
			...
			card_expiry = '--/--'
		print '\t', profile_id, profile_first, profile_last, profile_company, 'CC:', card_number, card_expiry
	...
	for h in sorted(history, key=lambda v: v['occurred_at']):
		...
		print '\t', history_id, history_type, history_action, history_occurred_at, history_amount, history_currency, history_description
